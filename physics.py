...
#Problem 8 Part 1
def calculate_auv_acceleration(F_magnitude, F_angle, mass):     #Newtons, radians, kg       Keep in mind the angle should not be greater/less than 30/-30 degrees (so pi/6 or -pi/6 rad)
    ...
    return np.linalg.norm(accelo_comp_force/mass)

print(calculate_auv_acceleration(100, 30, 100))

#Problem 8 Part 2
def calculate_auv_angular_acceleration(F_magnitude, F_angle, inertia, thruster_distance):        #Newtons, radians, kg*m^2, m
...
def calculate_auv2_acceleration(T, alpha, theta, mass):     #array, radians, radians, kg (100 default)
    ...
    auv_x_comp = (T[0]*np.cos(alpha)) + (T[1]*np.cos(alpha)) + (-1*T[2]*np.cos(alpha)) + (-1*T[3]*np.cos(alpha))      #T1, T2, T3, T4 - in that order. Calculate combination of component force in x direction. Relative to AUV.
    auv_y_comp = (T[0]*np.sin(alpha)) + (-1*T[1]*np.sin(alpha)) + (-1*T[2]*np.sin(alpha)) + (T[3]*np.sin(alpha))       #T1, T2, T3, T4 - in that order. Calculate combination of component force in y direction. Relative to AUV.
    auv_comp_forces = np.array(
        [auv_x_comp, auv_y_comp]
    )
    auv_accelo = np.linalg.norm(auv_comp_forces/mass)       #Combined acceleration of components. Relative to AUV
    auv_accelo_angle = np.arctan2(auv_y_comp, auv_x_comp)     #Angle of above acceleration. Relative to AUV

    auv_accelo_real_angle = theta + auv_accelo_angle        #Actual angle of the acceleration with respect to OG axes

    print("The AUV is accelerating at a rate of " + str(auv_accelo) + " m/s^2 at an angle of " + str(auv_accelo_real_angle) + " radians.")
    return auv_accelo, auv_accelo_real_angle
...
def calculate_auv2_angular_acceleration(T, alpha, L, l, inertia):        #array, radians, meters (x-direction), meters (y-direction), kg*m^2 (default 100)
    ...
    print("The AUV's torque is " + str(total_torque) + " rad/s^2.")
    return total_torque
...
def simulate_auv2_motion(T, alpha, L ,l ,mass, inertia, dt, t_final, x0, y0, theta0):
    ...
    for i in np.arange(0, t_final, dt):
        ...
        torque = calculate_auv2_angular_acceleration(T, alpha, L, l, inertia)        #Gives total_torque

        displacement = 0.5*accelo*(dt**2)           #Tells how much moved before it's broken down
